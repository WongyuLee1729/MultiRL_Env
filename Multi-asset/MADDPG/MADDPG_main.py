...
from Engine_multi_asset_Env import CryptoTradingEnv
...
def get_env(end_date, count, ticker): # 에피소드의 길이ep_len를 kwargs 형태로 env에 넘겨 줌 
    """
    1. Define environment 
    2. Determine observation space & action space for each agent 
    """
    # candle_data = SimulationDataProvider("ETH")
    new_env = CryptoTradingEnv(ticker, args.count)

    new_env.reset(end_date, count, ticker)
    _dim_info = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode_num', type=int, default=200,help='total episode num during training procedure')
    ...
    parser.add_argument("--count", type=int, default = 1000, help="size of the Data")
    ...
    env, dim_info = get_env(args.end_date, args.count, tickers)
    ...
    for episode in range(args.episode_num):
        obs = env.reset(args.end_date, args.count, tickers) # 날짜와 count dynamic하게 변하도록 수정할것!! 
