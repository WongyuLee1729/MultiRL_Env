...
from DateConvert import DateConverter
...
class DataRepository:
    ...
    - DataRepository에서 외부로 노출되는 메서드는 get_data 하나 뿐임
     => get_data는 시작지점부터 종료지점까지 전달받은 암호화폐의 거래 데이터를 모두 전달하는 역할을 함 
    - 데이터를 서버에서 가져올 지, 서버의 사용제한은 어떻게 회피할 지, 서버에서 가져온 데이터의 결함을 어떻게 처리할지는 
      모두 DataRepository 내부에서 처리함 
    """
    ...
    def __init__(self, db_file=None):
        ...
        self.verify_mode = False # DB에 저장된 값과 API로 가져온 값이 동일한지 검증 유무 
    ...
    def get_data(self, start, end, market="KRW-BTC"):
        ...
        count_info = DateConverter.to_end_min(start_iso=start, end_iso=end, max_count=100000000) # 시작-종료시점을 분단위 계산하여 몇개의 데이터가 존재하는지 파악
        total_count = count_info[0][2]
        db_data = self._query(start, end, market) # DB에서 동일한 기간에 대해 데이터 조회

        self.logger.info(f"total vs database: {total_count} vs {len(db_data)}")
        if len(db_data) > total_count: # 앞서 계산한 데이터의 숫자와 쿼리에서 조회한 데이터의 개수를 비교 
            raise UserWarning("Something wrong in DB") # 전체 데이터보다 db의 데이터가 큰 경우는 있을 수 없는 경우이므로 경고 메세지 발생

        if not self.verify_mode and total_count == len(db_data): # 동일한 기간 내에 데이터베이스에서 검색된 데이터의 개수와 같다면 데이터가 모두 데이터베이스에서 조회된 것으로 판단
            print('Data from DB')
            self.logger.info(f"from database: {total_count}")
            self._convert_to_upbit_datetime_string(db_data)
            return db_data
        print('Data from API')
        server_data = self._fetch_from_upbit(start, end, market) # verify_mode=True라면 server에서 가져온 데이터를 전달함 
    ...
    def _recovery_upbit_data(self, data, start, count, market, period=60):
        """
        Missing data가 존재할 경우 복구하는 코드 
        - 기준 시간을 기준으로 이동하며 빈곳이 존재하면 이전 데이터를 채워 넣은 뒤 DB 업데이트
        - recovered 필드:1 인 경우 복구된 데이터라고 명시 
        """
        new_data = []
    ...
    def _fetch_from_upbit_up_to_200(self, end, count, market):
        """
        업비트에서 데이터를 가져오기 위한 함수 
        -> 실제로 가져오는 부분은 _fetch_from_upbit_up_to_200_impl
        -> API 호출로 발생할 수 있는 에러 처리를 해주면서 while loop으로 데이터 수신이 완료 될 때 까지 요청함 
        """
        result = None
...
if __name__ == '__main__' :
    ...
    dr = DataRepository()
    dr.get_data("2020-02-20T14:02:00", "2020-02-27T23:51:00", "KRW-ETH") # 200개 이상 조회     
    
    # dr.get_data("2020-02-20T14:02:00", "2020-02-20T23:51:00", "KRW-BTC") # 200개 이상 조회 
    # dr.get_data("2020-02-20T17:00:00", "2020-02-20T22:10:00", "KRW-BTC") # 데이터베이스 데이터 사용 확인
    # dr.get_data("2020-02-22T03:54:00", "2020-02-22T03:56:00", "KRW-BTC") # upbit에 없는 데이터가 있음 확인 
    # result = dr.get_data("2020-01-10T00:00:00", "2020-01-20T00:00:00", "KRW-BTC") # 10,000개 이상 대량 데이터 테스트 
    # len(result)
