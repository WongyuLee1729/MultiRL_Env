'''
**** Changed the code for multi-asset ****
...
from SimDataProvider import SimulationDataProvider
...
class CryptoTradingEnv(AECEnv):
    ...
    def __init__(self, ticker, count):
        super().__init__()
        '''
        init method gets Env arguments and define the following attributes 
        - possible_agents
        - render_mode
        
        These attributes should not be changed after initialization
        '''
        # self.candle_data = [SimulationDataProvider(i) for i in tickers]
        
        self.candle_data = {i:SimulationDataProvider(i) for i in ticker }
        
        # self.agents = [f"agent_{i}" for i in range(3)]  # 에이전트 수 예시
        self.agents = ticker.copy()
        self.possible_agents = self.agents[:]
        self.agent_name_mapping = dict(zip(self.agents, list(range(3))))
    ...
    def reset(self, end_date,count, ticker):
        '''
        ...
        [self.candle_data[x].initialize_simulation(end=end_date, count=count) for x in self.agents]
        ...
        for i in self.agents:
            data = self.candle_data[i].get_info()
        ...
        self.set_observation_spaces(len(obs))
...
if __name__ == "__main__":
    ...
    # candle from data engine 
    
    # Create Env
    env = CryptoTradingEnv(tickers, opt.count)

    env.reset(end_date=end_str, count = opt.count, ticker = tickers)
