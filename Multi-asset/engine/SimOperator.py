...
from GenOperator import Operator
import pdb
...
class SimulationOperator(Operator):
    ...
    def _execute_trading(self, task):
        ...
        try: 
            ...
            self.logger.error("excuting failed")
...
if __name__ == "__main__":
    ...
    from SimTrader import SimulationTrader
    from StrBnH import StrategyBuyAndHold
    ...
    parser.add_argument("--end_date", type=str, default = "2020-04-30T00:00:00", help="End date of data")
    parser.add_argument("--count", type=int, default = 20, help="size of the Data")
    ...
    data_provider.initialize_simulation(end=end_str, count=opt.count) # API를 통해 데이터를 가져옴 QUERY_STRING = {"market": "KRW-BTC"} 는 변수로 변경해줄 것!
    trader = SimulationTrader()
    trader.initialize_simulation(end=end_str, count=opt.count, budget=opt.budget)
    analyzer = Analyzer()
    ...
    operator.set_interval(opt.interval)
    operator.start()
