...
if __name__ == '__main__':
    ...
    for episode in range(args.episode_num):
        obs = env.reset() # 맨 처음에는 환경을 리셋하며 obs를 받아오고 loop를 돌면서는 이전 obs인 next_obs를 assign 해 줌 obs = next_obs
        ...
        while env.agents:  # interact with the env for an episode
            ...
            if step >= args.random_steps and step % args.learn_interval == 0:  # learn every few steps
                maac.learn(args.batch_size, args.gamma) # !!!MAAC!!!
                maac.update_target(args.tau) # !!!MAAC!!!
                
                # maddpg.learn(args.batch_size, args.gamma)
                # maddpg.update_target(args.tau) # target network
    ...
    maac.save(episode_rewards) # save model
